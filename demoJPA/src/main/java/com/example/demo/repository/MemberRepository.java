...
// => DML 사용시에는 @Modifying, @Transactional 반드시 사용. (스키마가 바뀌는 건 DDL, 인스턴스가 바뀌는 건 DML)
// => select는 해당 없고, insert, update, delete는 @Modifying과 @Transactional 반드시!!! 
...
public interface MemberRepository 
		...
		// 2) @Query를 이용한 직접 쿼리 선언
		// => password Update에 적용
		// 2.1) JPQL
		// => table 명 대신 Entity
		@Modifying
		@Transactional
		@Query("update Member set password=:pw where id=:ii")
		void updatePassword1(@Param("ii") String id, @Param("pw") String password);
		
		@Modifying
		@Transactional
		// 2.2) Native_Query 적용
		// => nativeQuery 속성 true, Table명 사용
		@Query(value = "update member set password=:pw where id=:ii"
			 , nativeQuery = true)
		void updatePassword2(@Param("ii") String id, @Param("pw") String password);
