...
@Table(name = "member") // Mysql Table 'member'와 매핑될 예정
...
public class Member {
	...
	@Transient // DB에는 없는 필드(컬럼)명 ; sql 구문 처리 시 제외시켜줌 (하이버네이트 properties ddl-auto를 한다고 하더라도)
	private MultipartFile uploadfilef; // application.properties에서 auto설정을 안했기 때문에 다행히 mysql에 column 추가가 안됨
	   /*
