...
public class GuestbookServiceImpl implements GuestbookService {
	...
	//    final은 초기값이 고정값인데, 인터페이스라 생성자 메소드(초기값 설정)를 쓰기 어려움
	// => 생성자 주입 어노테이션 (@RequiredArgsConstructor를 통해 주입받음 ※@Autowired는 MyBatis 때)
	...
	public PageResultDTO<GuestbookDTO, Guestbook> pageList(PageRequestDTO requestDTO) {
		...
		//    람다식을 인자로 직접 사용해도 됨 (이로써 Java도 함수형 언어라 할 수 있게 되었다.)
	...
	public Guestbook selectOne(long gno) {
        ...
        // => isPresent() : Optional 객체에 저장된 값이 null인지 확인 
		...
		// Optional<Guestbook> : 람다 - 스트림 - optional ;
	}
