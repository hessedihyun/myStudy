...
//=> 주요기능
// - Page<Entity> 객체들을 DTO 객체로 변환해서 List에 담아줌 => '생성자'에서 진행
// - 화면출력을 위한 페이지 정보들 구성
...
public class PageResultDTO<DTO, EN> {
    ...
    // Generic타입 클래스는 생성자 메소드에 Generic을 정의하지 않는다.
    // new ArrayList(); [O] VS new ArrayList<>(); [X]
    	
        dtolist = result.stream().map(fn).collect(Collectors.toList());
        // ※ file class, I/O stream(메모리에서 하드웨어랑 연결되는 것 ex. 이미지 값 복사, 위치 변경)
        //   , 일반 스트림(메모리 상에서 벌어난 '흐름'. 순차 처리가 가능한 여러 데이터를 배열, 컬렉션 등으로 가지고 있을 경우)
        // ※ 자바의 stream에서는 chain 함수, 최종 함수가 있다.
        //=> stream()
