...
public class PageMaker {
	...
	// => bCheckList 또는 bPageList 등 요청명에 해당하는 url을 만들 수 있도록 하기 위함
	...
	// => 이 값을 이용해서 나머지 필요한 값 계산 (별도의 메소드 만들기)
	...
	public void calcData() {
		// 3.1) spageNo, epageNo
		// => currPage가 속한 PageBlock을 찾아서 spageNo, epageNo 계산
		...
		// => 요청받은 currPage 11인 경우라면 ? 
		// -> epageNo: (int)Math.ceil(11/3) * 3 = 12 
		// -> spageNo: 12 - 3 + 1 = 10
		// -> 결론 : 11은 10, 11, 12 그룹에 속함
	...
	public String makeQuery(int currPage) {
     ...
     // => ?curPage=1&rowsPerPage=5&searchType=title&keyword=Java&check=admin&check=banana
	 ...
	 public String searchQuery(int currPage) {
	     ...
	     // => MultiValueMap 생성
		 // 왼쪽(MultiValueMap) : 인터페이스, 오른쪽(LinkedMultiValueMap) : 구현체
	     MultiValueMap<String, String> checkMap = new LinkedMultiValueMap<String, String>(); 
	     
	     // MultiValueMap 과 HashMap의 차이 : 
	     // MultiValueMap은 키에 여러 개의 값을 저장할 수 있는 반면, HashMap은 키에 하나의 값만 저장할 수 있다.
	     // map.add("A",100);
	     // List<Integer> a = map.get("A");
	     // for(int data: a) { System.out.print(data + " ")};
	     // HashMap은 값을 덮어버림
	     // LinkedHashMap은 HashMap 기능 + 입력 순서 보장 
