...
// ** interface 1.
//=> 상수, 추상메서드
//=> static, default 메서드 (구현부가 있는 메서드, Java8 부터 추가됨) // 추상abstract으로 만들면 오류가 난다.
...
public class Ex02_InterAnimal {
	...
	public static void main(String[] args) {
		...
		//c2=c1; X   //컴파일오류, Cati c2 = new Animali();  와 동일구문
		//c2=animal; X //컴파일오류, Cati c2 = new Animali();  와 동일구문
		//c1=c2; O (자동형변환)
		...
		if ( bear instanceof Animali ) { 
			// instanceof A에서 A가 추상클래스/일반클래스라면, 오류가 나는 경우 바로 컴파일 오류에서 확인 가능하다. 
			// 인터페이스는 컴파일 타입에서 확인이 안되기 때문에 잡히지 않는다. 런타임 시에 오류를 확인할 수 있다.
