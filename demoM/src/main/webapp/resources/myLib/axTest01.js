...
function rsLoginf() {
	...
	`<table align center>
		...
		<tr height=40>
			<td bgcolor="LightSalmon"><label for="id">ID</label></td>
			<td><input type="text" name="id" id="id" placeholder="ID를 여기에 적어주세요"/></td>
		</tr>
		<tr height=40>
			<td bgcolor="LightSalmon"><label for="password">Password</label></td>
			<td><input type="password" name="password" id="password" placeholder="영문,숫자,특수문자 10자 내외"/></td>
		</tr>
		<tr height=40>
			<td></td>
...
function rsLogin() {
	...
	fetch(url,{method:'Post'
       ...
       // => Response Body의 Data-reading & return.
	    ...
	    return response.text(); // body를 reading하는 함수를 통해 다음 then 단계로 넘어감
       ...
       //    (※ fetch - then - then - catch VS axios - then - catch)
       ...
       // => 1단계에서 return 한 Data 처리
		...
		if(err.message=='502') alert('id 또는 password 오류입니다. 재시도하세요.');
		else alert(`System 오류가 발생했습니다. 잠시 후 다시 시도하세요.=> ${err.message}`);
	});
} // rsLogin
...
function rsLoginjj() {
	...
	fetch(url,{method:'Post'
		...
		if(!response.ok) throw new Error(response.status);
	    return response.json(); // body를 reading하는 함수를 통해 다음 then 단계로 넘어감
...
function axiLoginjj() {
	...
	axios({url: url
		 , method:'Post'
		 , headers:{'Content-Type':'application/json'}
		 , data: {id: document.getElementById('id').value // axios: body 대신 data라 부른다.
				, password: document.getElementById('password').value
		        }
	}).then(response=> {
		...
		else alert(`System 오류입니다. err.message(status) => ${err.message}`)
	});
} // axiLoginjj
// ***************************************************************
// *** JSON 객체
