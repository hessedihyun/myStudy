...
function idCheck() {
   ...
   if(id.length<4 || id.length>10){
      ...
      return false;
   // => 영문과 숫자로만 입력했는지 : id 에서 영문과 숫자를 모두 ''(없음)로 변경(replace)했을때 length 가 0 이면 OK   
      } else if(id.replace(special,'').length>0) {
   // => test(검사대상문자) 메서드 활용(불규칙 적용 => Replace 권장)
...
function pwCheck() {
   ...
   if(pw.length<4 || pw.length>10){
      ...
      return false;
   } else if(pw.replace(/[!-*.@]/gi,'').length >= pw.length) { // => 특수문자는 반드시 포함
