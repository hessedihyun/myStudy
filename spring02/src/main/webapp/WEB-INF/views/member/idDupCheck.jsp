...
function idOK(){
	...
	// opener.document.getElementById('id').value=document.getElementById('id').value;
	   ...
	   // => EL 활용 : jsp 문서에서는 EL테그 언어를 JS는 모르지만, tomcat 서버가 script 구문의 문자열 내부에 있는 EL은 읽어주고 처리해줌
	   //             -> 스트링으로 확인해야 하기 때문에 백틱 안됨. 만약 그냥 =${param.id}이라고 쓰면 '제이쿼리'로 읽는다.
	   // ※ 제이쿼리란? : window.jQuery.attr(); -> $.attr() (단, 외부소스이기 때문에 제이쿼리 라이브러리를 꼭 css, js처럼 넣어줘야함)
	// 2)
...
<div id="wrap">
   ...
   <div id="msgBlock">
   		...
   		<c:if test="${idUse=='F'}">
   			${param.id}는(은) 사용 불가능합니다. (이미 사용중입니다.)<br>
   			다시 입력하세요.<br>
