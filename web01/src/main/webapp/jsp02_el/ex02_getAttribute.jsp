...
// => 호출, 우선순위 
   pageContext.setAttribute("name", "pageContext Value1"); // page의 원형 객체가 pageContext라고 했엇다.
...
<body>
	...
	=> 동일한 속성명을 사용한 경우에 모두 구별해서 출력하려면 
	   속성명을 붙여 적용 (앞에 영역_Scope 객체가 생략된 것임)
	...
	2) 서로 다른 속성(Attribute)명을 모든 영역에 정의
	=> 속성명만 사용하여 출력가능
	=> 그러나 영역(Scope)을 붙여주는 것이 효율적이다. (direct로 인식)
	\${pname} => ${pname}
	\${rname} => ${rname}
	\${sname} => ${sname}
	\${aname} => ${aname}
	<hr>
	3) 연산자 Test
	=> request 영역에 속성(Attribute) 2개를 만들고 활용 Test
	3.1) Java
	<% int n1 = (Integer)request.getAttribute("num1"); // Object 객체를 int로 형 변환%>
	...
	<% n1= Integer.parseInt(request.getParameter("num1")); // String을 int로 형 변환 
